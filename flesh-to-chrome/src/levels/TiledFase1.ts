...
/** nome do tileset dentro do próprio JSON (Tiled `tileset.name`) - exigido por `map.addTilesetImage` */
export const TILED_FASE1_TILESET_NAME = "sewer";
 ...
 * tela. O grid do Tiled é 16×16 e o mapa tem só 20 linhas (320px) de
 * altura, mas a viewport é 720px, e este motor **não tem scroll vertical
 * de câmera** (`GameScene.update()` só ajusta `scrollX` - confirmado sem
 ...
 *
 * Escolhido para a linha 16 do grid (o chão principal, y=256 no espaço do
 * mapa) cair em y=560 na tela - o mesmo `GROUND_Y` usado pelo nível
 * desenhado à mão (`phase1.ts`), mantendo o enquadramento familiar e a
 * mesma folga vertical pro pulo (altura máxima 128px desde a v0.3.0, bem
 * menor que os 560px de espaço livre acima).
 */
export const TILED_FASE1_OFFSET_Y = 560 - 16 * 16; // = 304
