...
Esta especificação mantém os requisitos da disciplina e acrescenta os requisitos do produto. **Documentado não significa implementado.** Conforme confirmação do responsável com o professor, não há exigência de metas de latência ou FPS para a entrega final;
...
| RF-ISN-09 | Catálogo de eventos críticos e persistência de auditoria; RN10. D06: campanha registrada a partir da autenticação, sem histórico retroativo das ações de visitante. |
...
| RF-J04 | Manter um save por conta autenticada, com cache local, continuar do checkpoint e confirmar a substituição por Novo Jogo. Visitante mantém progresso apenas na sessão e o perde ao fechar/recarregar o jogo. | RN01/03; UC04/09. |
| RF-J05 | Vincular o progresso iniciado como visitante à conta após login, conforme reconciliação D05, e restringir acesso ao proprietário; sem login não há persistência da campanha. | UC02/04; D05 aprovada: escolha explícita, revisão e isolamento de cache por conta. |
...
| RNF02 — gameplay | Manter a campanha jogável; chamadas de save/e-mail não bloqueiam a simulação. Sem meta obrigatória de FPS. | Percorrer uma fase, verificar controles e continuidade com rede lenta ou indisponível. Medições de frame time são opcionais para diagnóstico. |
| RNF03 — API | Ler/gravar save corretamente, informar carregamento e tratar falhas sem perder progresso. Sem prazo numérico de resposta exigido para a entrega. | Verificar respostas, autorização, conflitos e indisponibilidade; medir tempos apenas se útil para investigar problemas. |
| RNF04 — multiplayer | Concluir corrida com sincronização funcional e resultado conforme RN08. Sem limite obrigatório de RTT/latência/FPS para a entrega. | Testar salas, prontidão, corrida, resultado e desconexão conforme documento 13. Taxa de snapshots, detecção de queda e carga de operação continuam em D04; medições auxiliam diagnóstico, sem mínimo acadêmico numérico. |
| RNF05 — custo | Usar arquitetura serverless/gerenciada do documento 12 e estimar custo bruto, créditos e franquias separadamente; definir teto antes de provisionar. | Planilha/estimativa por carga e alarmes de orçamento; teto e dimensionamento em D08; comportamento multiplayer em D04. Região `sa-east-1` é previsão, não comprovação de menor custo. |
